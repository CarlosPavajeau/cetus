...
import { createStoreMiddleware } from './interceptors/current-store'
...
export class HttpClient implements FetchClient {
  ...
  constructor(config: ApiClientConfig) {
    const { baseURL, auth, store, timeout = 10_000, headers = {} } = config
    ...
    let finalFetch: <T>(config: FetchRequestConfig) => Promise<T> = auth
      ? createAuthMiddleware(baseFetch, auth)
      : baseFetch

    if (store) {
      finalFetch = createStoreMiddleware(finalFetch, store)
    }

    this._request = finalFetch
  }
